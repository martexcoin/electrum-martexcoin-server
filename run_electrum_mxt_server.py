...
if os.path.dirname(os.path.realpath(__file__)) == os.getcwd():
    imp.load_module('electrummxtserver', *imp.find_module('src'))

from electrummxtserver import storage, networks, utils
from electrummxtserver.processor import Dispatcher, print_log
from electrummxtserver.server_processor import ServerProcessor
from electrummxtserver.blockchain_processor import BlockchainProcessor
from electrummxtserver.stratum_tcp import TcpServer
from electrummxtserver.stratum_http import HttpServer
...
def create_config(filename=None):
    ...
    config.set('server', 'banner_file', '/etc/electrum-mxt.banner')
    ...
    config.set('server', 'coin', 'martexcoin')
    config.set('server', 'logfile', '/var/log/electrum-mxt.log')
    config.set('server', 'donation_address', '')
    config.set('server', 'max_subscriptions', '10000')

    config.add_section('leveldb')
    config.set('leveldb', 'path', '/dev/shm/electrum-mxt_db')
    ...
    config.set('network', 'type', 'martexcoin_main')

    # try to find the config file in the default paths
    if not filename:
        for path in ('/etc/', ''):
            filename = path + 'electrum-mxt.conf'
